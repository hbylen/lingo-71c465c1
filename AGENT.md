...
│   ├── App.tsx                    # 主路由/布局容器，已添加basename支持
│   ├── main.tsx                   # 应用入口
│   ├── index.css                  # 全局样式（含 Tailwind 指令）
│   └── mock.json                  # 模拟数据
│
├── package.json                   # 项目依赖和脚本配置
├── vite.config.ts                 # Vite 构建工具配置，已设置base: './'解决路径问题
├── tsconfig.json                  # TypeScript 主配置
├── tsconfig.node.json             # TypeScript 节点配置
├── tailwind.config.js             # Tailwind CSS 配置
├── postcss.config.js              # PostCSS 配置
└── index.html                     # HTML 入口文件，已修正资源路径为相对路径
```
...
- `vite.config.ts`: Vite 构建工具配置，包含 react 插件、服务器配置（host: true, allowedHosts: true）以及 **base: './'** 设置，解决部署时的路径问题
...
- `index.html`: HTML 入口文件，资源路径已修正为相对路径（./vite.svg, ./assets/main.js）

### 源代码文件
- `src/main.tsx`: React 应用入口点，渲染 App 组件
- `src/index.css`: 全局样式文件，包含 Tailwind 指令和自定义样式
- `src/App.tsx`: 主应用组件，设置路由和基本布局，**已添加 basename={import.meta.env.BASE_URL} 支持相对路径**
